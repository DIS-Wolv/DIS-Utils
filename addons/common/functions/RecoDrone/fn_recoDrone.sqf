...
params [["_droneType", "B_UAV_02_F"], ["_range", 2000], ["_altitude", 2000], ["_posBase", [0,0,2000]], ["_isArmed", false], ["_redeploy", -1]];
...
private _fnc_RTB = {
	...
	if (_redeploy > 0) then {
		...
		private _isArmed = _params select 3;
		private _posBase = _params select 4;
		...
		[_droneType, _range, _altitude, _posBase, _isArmed, _redeploy] call DISCommon_fnc_recoDrone;
	};
};
...

{
	...
	//WP de retour
	[_ThisDrone, _spawnPos, [_droneType, _range, _altitude, _posBase, _isArmed, _redeploy]] spawn _fnc_RTB;
