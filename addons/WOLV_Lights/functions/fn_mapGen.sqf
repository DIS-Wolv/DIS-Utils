...

{
	...
	switch (_power) do {
		case 4: {	// 3 affiche les générateur et le reseaux
			...
			private _countPetit = 0;
			private _countMoyen = 0;
			private _countGrand = 0;
			...
			{		//pour chaque petit poteau
				...
				if(_isPetitPoteaux != -1) then {  	//si c'est un petit poteaux
					...
					if (_countPetit <= 1) then {
						_map ctrlAddEventHandler ["Draw",
							format["(_this select 0) drawLine [%1,%2,[0,0.8,0,1]];", str(_posPoteau), str(_posG)]
						];
						_countPetit = _countPetit + 1;
					};
				...
				if(_isMoyenPoteaux != -1) then {  	//si c'est un petit poteaux
					_posPoteau = (position _x); 	// recupère ca position et appel le script
					if (_countMoyen <= 1) then {
						_map ctrlAddEventHandler ["Draw",
							format["(_this select 0) drawLine [%1,%2,[0,0,1,1]];", str(_posPoteau), str(_posG)]
						];
						_countMoyen = _countMoyen + 1;
					};
				...
				if(_isGrandPoteaux != -1) then {  	//si c'est un petit poteaux
					_posPoteau = (position _x); 	//recupère ca position et appel le script
					if (_countGrand <= 1) then {
						_map ctrlAddEventHandler ["Draw",
							format["(_this select 0) drawLine [%1,%2,[0.85,0.4,0,1]];", str(_posPoteau), str(_posG)]
						];
						_countGrand = _countGrand + 1;
					};
