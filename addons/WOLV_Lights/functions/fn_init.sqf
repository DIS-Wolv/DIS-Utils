...
private _infoOff = "Hide Generator";
private _initMessage = "Light script by [DIS]Wolv initialized";
// get the lang
if (!isNull _logic) then {
	private _infoLang = _logic getVariable ["Lang", 0];
	// set the langs
	switch (_infoLang) do {
		case 1: {
			_infoOn = "Afficher Générateur";
			_infoOff = "Cacher Générateur";
			_initMessage = "Script de lumière par [DIS]Wolv initialisé";
		};
		case 0;
		default {
			_infoOn = "Display Generator";
			_infoOff = "Hide Generator";
			_initMessage = "Light script by [DIS]Wolv initialized";
		};
	};
};
...
if (!isNull _logic) then {
	private _infoName = _logic getVariable ["Name", ""];

	// get the info object
	private _Info = missionNamespace getVariable _infoName;
